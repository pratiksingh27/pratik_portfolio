...
//  custome link is made to designe link with single component
...
{/* //responsive  navbar design */}
    <button className='flex-col justify-center items-center mt-2 hidden lg:flex' onClick={handleClick}>
        <span className={`bg-dark dark:bg-light block transition-all duration-75 ease-in-out h-0.5 w-6 rounded-sm ${isOpen ? 'rotate-45 translate-y-1' : '-translate-y-0.5'}`}></span>
        <span className={`bg-dark dark:bg-light block transition-all duration-75 ease-in-out h-0.5 w-6 rounded-sm my-0.5 ${isOpen ? 'opacity-0' : 'opacity-1'}`}></span>
        <span className={`bg-dark dark:bg-light block transition-all duration-75 ease-in-out h-0.5 w-6 rounded-sm ${isOpen ? '-rotate-45 -translate-y-1' : 'translate-y-0.5'}`}></span>
    </button>
...

    {
        ...
        className='min-w-[70vw] flex flex-col justify-between items-center fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-40 bg-dark/90 dark:bg-light/75 rounded-lg backdrop-blur-md py-32'>
        <nav className='flex items-center flex-col justify-center'>
            ...
            <nav className='flex items-center justify-center flex-wrap mt-2'>
                ...
                <motion.a href="https://twitter.com" target={"_blank"} className='w-6 mx-3 bg-light rounded-full dark:bg-dark sm:mx-1'
                 whileHover={{y:-2}}
                 whileTap={{scale:0.95}}>
                    <GithubIcon />
                </motion.a>
        ...
        <div className='absolute left-[50%] items-center translate-x-[-50%]'>
        <Logo/>
        </div>
    </header>
  )
